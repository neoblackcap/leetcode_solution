...
class BinaryTreeBuilder {
 ...
 public:
  ...
  vector<int> *serialize(InvertTreeNode *root) {
    _serialize_queue.push_back(root);
...
TEST(InvertBinaryTreeTestCase, test_solution) {
  ...
  InvertTreeNode *root_another = builder.build_tree();
  solution.invertTree(root);
  vector<int> *actal_result = builder.serialize(root);
  vector<int> *actal_result_another = builder.serialize(root_anothor);
  vector<int> except_result = {4, 7, 2, 9, 6, 3, 1};
  EXPECT_EQ(*actal_result, except_result);
  EXPECT_EQ(*actal_result_another, except_result);
}
